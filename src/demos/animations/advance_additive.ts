...
// 场景基本的构建方法
export const advanceAdditiveAnmiationDemo = function (engine:BABYLON.Engine, canvas:HTMLCanvasElement) {
	...
	// Model by Mixamo
	BABYLON.ImportMeshAsync("https://playground.babylonjs.com//scenes/Xbot.glb", scene).then((result)=>{
		...
		// 初始化动画，设置idle w为默认动画
		var idleAnim = scene.animationGroups.find(a => a.name === 'idle') as BABYLON.AnimationGroup;
		var idleParam = { name: "Idle", anim: idleAnim, weight: 1 };
        idleAnim.weight = 1;
		idleAnim.play(true);

		var walkAnim = scene.animationGroups.find(a => a.name === 'walk') as BABYLON.AnimationGroup;
		var walkParam = { name: "Walk", anim: walkAnim, weight: 0 };
        walkAnim.weight = 0;
		walkAnim.play(true);

		var runAnim = scene.animationGroups.find(a => a.name === 'run') as BABYLON.AnimationGroup;
		var runParam = { name: "Run", anim: runAnim, weight: 0 };
        runAnim.weight = 0;
		runAnim.play(true);

		// Initialize additive poses. Slice of reference pose at first frame
		var sadPoseAnim = BABYLON.AnimationGroup.MakeAnimationAdditive(
            scene.animationGroups.find(a => a.name === 'sad_pose') as BABYLON.AnimationGroup
        );
		...
		var sneakPoseAnim = BABYLON.AnimationGroup.MakeAnimationAdditive(
            scene.animationGroups.find(a => a.name === 'sneak_pose') as BABYLON.AnimationGroup
        );
		...
		var headShakeAnim = BABYLON.AnimationGroup.MakeAnimationAdditive(
            scene.animationGroups.find(a => a.name === 'headShake') as BABYLON.AnimationGroup
        );
		var headShakeParam = { name: "Head Shake", anim: headShakeAnim, weight: 0. };
        headShakeAnim.weight = 0;
		headShakeAnim.play(true);

		var agreeAnim = BABYLON.AnimationGroup.MakeAnimationAdditive(
            scene.animationGroups.find(a => a.name === 'agree') as BABYLON.AnimationGroup
        );
		...
		var currentParam : { name: string, anim: BABYLON.AnimationGroup, weight: number } | undefined = idleParam;
